...
ConnectionPool pool = ConnectionPool.getInstance(); //객체를 생성 이 순간 10개가 촥 올라올거야
...
int counter = 0;
try {
	conn = pool.getConnection(); // 빌리는 거
			//DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:xe", "douzone", "1234");
...
<body bgcolor="#FFFFCC">
	...
	if (conn != null)
		try {
			//conn.close();
			pool.releaseConnection(conn); //반납
		} catch (Exception ex) {
		}
	}
	%>
